...
public class Structure {
    ...
    public synchronized static Structure getInstance() {
        if (structure == null) {
     ...
     * 素往后移一位，为元素 x “腾位置”，最后将 k 对应的元素值赋为 x ，一般情况下，插入排序的时间
     * 复杂度和空间复杂度分别为 O(n2 ) 和 O(1)。
     *
     * @param array 未排序数组
     * @return int[] 排完序数组
     */
     ...
     * 2.选择排序算法
     * <p>
     * 选择排序的基本思想是遍历数组的过程中，以 i 代表当前需要排序的序号，则需要在剩余的
     * [i…n-1] 中找出其中的最小值，然后将找到的最小值与 i 指向的值进行交换。因为每一趟
     * 确定元素的过程中都会有一个选择最大值的子流程，所以人们形象地称之为选择排序。选择排
     * 序的时间复杂度和空间复杂度分别为 O(n2 ) 和 O(1) 。
     *
     * @param arr 未排序数组
     * @return int[] 排完序数组
     */
     ...
     * 4.快速排序算法
     * <p>
     * 通过一趟排序将待排记录分割成独立的两部分,其中一部分记录的关键字均比另一部分的关键字小,则可以分别对这两部分记录继续进行排序,已达到整个序列有序的目的，本质就是,
     * <p>
     * 找一个基位(枢轴, 分水岭, 作用是左边的都比它小, 右边的都比它大.可随机, 取名base，首先从序列最右边开始找比base小的, 如果小, 换位置, 从而base移到刚才右边(比较时比base小)的位置(记为临时的high位),这样base右边的都比base大。然后,从序列的最左边开始找比base大的，如果大,换位置,
     * <p>
     * 从而base移动到刚才左边(比较时比base大)的位置(记为临时的low位),这样base左边的都比base小，循环以上两步,
     * 直到 low ==heigh,这使才真正的找到了枢轴,分水岭.返回这个位置,分水岭左边和右边的序列,分别再来递归。
     *
     * @param array 未排序数组
     * @return int[] 排完序数组
     */
     ...
     * 5.合并排序算法
     * <p>
     * 归并排序采用的是递归来实现，属于“分而治之”，将目标数组从中间一分为二，之后分
     * 别对这两个数组进行排序，排序完毕之后再将排好序的两个数组“归并”到一起，归并排
     ...
     * 以 gap
     * 来划分，比如数组 [1,2,3,4,5,6,7,8] ，
     * 如果以 gap = 2
     * 来划分，可以分为 [1,3,5,7]和 [2,4,6,8]两个数组（对应的，
     * 如 gap = 3 ，则划分的数组为： [1,4,7] 、 [2,5,8] 、 [3,6] ）然后分别对划分出来的数组进行插入排序，
     * 待各个子数组排序完毕之后再减小 gap
     * 值重复进行之前的步骤，
     * 直至 gap = 1 ，即对整个数组进行插入排序，此时的数组已经基本上快排好序了，所以需要移动的元素会很小很小，解决了插入排序在处理大规模数组时较多移动次数的问题，
     * <p>
     ...
     * 7.堆排序算法
     * <p>
     * 本质就是先构造一个大顶堆,parent比children大,
     * <p>
     * root节点就是最大的节点 把最大的节点(root)与尾节点(最后一个节点,比较小)位置互换，
